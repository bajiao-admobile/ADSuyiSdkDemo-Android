...
针对激励视频广告类型，仅支持 Android 8.1.11 以上版本。可以设置服务端器回调，在用户观看完成视频后，服务端会回调奖励信息到您的服务器。

您可以根据需要选择通过sdk或者服务端回调信息。若需要设置服务端回调，可以在后台添加callbackURL，**Mintegral会在视频播放完成后调用callbackurl给予信息的回调。**

```java
/**
     * 汇量平台激励视频服务验证，支持透传的参数为自定义用户ID(String)和自定义参数(String)
     * 参数透传对应汇量SDK MBRewardVideoHandler.show(String userId, String extraData)
     */
    private void initAd() {
        // 创建激励视频广告实例
        rewardVodAd = new ADSuyiRewardVodAd(this);

        //对应汇量SDK -> show(String userId, String extraData)
        ADSuyiRewardExtra adSuyiRewardExtra = new ADSuyiRewardExtra("设置自定义用户ID-userId");
        adSuyiRewardExtra.setCustomData("设置自定义参数-extraData");
        ...
        rewardVodAd.setListener(new ADSuyiRewardVodAdListener() {
            ...
            @Override
            public void onReward(ADSuyiRewardVodAdInfo adSuyiRewardVodAdInfo) {
                String platformFlag = "mintegral"; //汇量平台标识
                if (platformFlag.equals(adSuyiRewardVodAdInfo.getPlatform())) {
                    Map<String, Object> rewardMap = adSuyiRewardVodAdInfo.getRewardMap();
                    if (rewardMap != null) {
                        //获取汇量RewardInfo参数信息
                        if(rewardMap.get("rewardInfo") != null){
                            com.mbridge.msdk.out.RewardInfo rewardInfo = (com.mbridge.msdk.out.RewardInfo) rewardMap.get("rewardVerify");
                        }
